...
gulp.task('copy', function () {
  ...
  md(DIR.dist)
  fs.readdirSync(DIR.source).filter(function (fp) {
    return !fs.statSync(path.join(DIR.source, fp)).isDirectory()
  }).forEach(function (file) {
    let src = fs.createReadStream(path.join(DIR.source, file))
    let dest = fs.createWriteStream(path.join(DIR.dist, file))
    src.pipe(dest)
  })
  fs.readdirSync(DIR.source).filter(function (dir) {
    return ['sass'].indexOf(dir.toLowerCase()) < 0 && fs.statSync(path.join(DIR.source, dir)).isDirectory()
